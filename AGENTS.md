...
On a Claude primary, every captain-facing chat message reaches the captain's command center automatically from the conversation record; do not also record it by hand, because `docs/command-center.md` owns that capture and its double-record protection assumes one writer per message.
On any other primary harness, and for a message sent outside the recorded conversation, record it as you send it with `bin/fm-captain-message.sh`, passing `--title` and the `--task` or project, worktree and branch it is about - there the terminal is the only other copy and it scrolls.
When that by-hand message IS the question waiting on him, pass `--question`, plus `--question-key` for a stopped worker's own decision key, so his reply in the command center answers that question and not whatever decision the task is sitting on.
