...
1. **Never write to a project.**
   Do not edit, commit, or run state-changing commands under `projects/` or in any project worktree; crewmates change projects, and the delegate guard refuses firstmate's own calls into one, reads included.
   ...
   Firstmate may directly edit, create, move, or delete project files or directories only when the captain clearly and concretely approves, in the moment, for a specific project, either a specific operation or a concrete scope whose authorized action needs no inference; firstmate performs exactly that approval through a crewmate it dispatches for it, because the delegate guard refuses the call in place whatever its approval and carries no release token, never infers or broadens it, and gains no standing authority, while the force, discard, unlanded-work, merge-authority, destructive, irreversible, and security-sensitive boundaries remain independently in force.
...
Tracked files hold shared instructions and tooling; `data/` holds durable private fleet records; `state/` holds runtime records and append-only status events; `config/` holds local operating choices; and `projects/` contains clones that firstmate does not open at all, not even to read, except under hard rule 1's concrete captain-approved project operation exception.
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  captain-messages.jsonl  append-only record of every message firstmate sent the captain; LOCAL, gitignored, written by the automatic conversation-record capture (bin/fm-captain-message-sweep.py) and by hand with bin/fm-captain-message.sh, read back by the command center (section 9; docs/command-center.md)
...
projects/            cloned repos; gitignored; never firstmate's to open, reads included, except under hard rule 1's concrete captain-approved project operation exception
state/               runtime records and signals; gitignored
  <id>.status        appended by crewmates: "<state>: [<utc-timestamp>] <note>" wake-event lines, not current-state truth; the note-head timestamp bracket is how long a line has been waiting, and a line without one still parses (bin/fm-classify-lib.sh owns the grammar)
  ...
  captain-hold-resolutions/ id-bound, decision-text-free receipts written only by bin/fm-captain-hold.sh answer; permanent evidence that a closed captain call was answered after its Done row is pruned (docs/captain-hold-lifecycle.md)
  ...
  .captain-message-sweep .captain-message-sweep.lock .captain-message-capture  automatic captain-message capture cursor (per transcript path, including which transcripts a Stop hook payload has named), lock (also the captain-message log's write lock, taken by bin/fm-captain-message.sh and bin/fm-captain-message-backfill.py), and health record; bin/fm-captain-message-sweep.py owns them, and deleting the cursor forces one full re-read, never a duplicate
...
The replacement work therefore starts on a fresh branch cut from the current default head, carries over the pre-invalidation content, leaves the obsolete run's own pipeline-fix commits behind, and abandons the recovered branch and any PR it opened rather than repairing it.
...
On a Claude primary, every captain-facing chat message reaches the captain's command center automatically from the conversation record (`docs/command-center.md`); that capture is the guarantee nothing you say goes missing, and it never marks a message as a question, so a reply to a captured message reaches you as a note.
When a message asks the captain something tied to a specific decision, also record it yourself with `bin/fm-captain-message.sh --question`, plus `--question-key` for a stopped worker's own decision key, passing `--title`, the `--task` it is about, and exactly the text of that turn's final message; that routed record is what makes his reply answer that decision, and the capture keeps it instead of adding a bare copy beside it.
On any other primary harness, and for a message sent outside the recorded conversation, record every captain-facing message the same way as you send it, with `--question` only when it is the question waiting on him - there the terminal is the only other copy and it scrolls.
Every by-hand record names the work it is about with `--task <id>` (the task's `state/<id>.meta`), or `--general` only for a message about no task; the recorder refuses one with neither, so the captain always sees its project, worktree and branch.
