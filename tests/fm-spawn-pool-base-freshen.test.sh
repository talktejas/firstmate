...
test_linked_spawning_home_rejects_primary_before_refresh() {
  ...
  for returned in primary primary-alias spawning scout; do
    ...
    if [ "$returned" = scout ]; then
      ...
      # None of these is an isolated copy, so the isolation screen refuses the
      # leased path before the pane is ever moved into it: the spawning
      # directory fails the project comparison, and the repository primary
      # (named directly or through a symlink) fails the common-git-dir test.
      # The refusal names the copy it rejected and why.
      ...
      assert_contains "$out" "resolved '$POOL_DIR'" \
        "refusal did not name the copy it rejected"
