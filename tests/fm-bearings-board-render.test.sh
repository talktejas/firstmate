...
test_an_underway_row_leads_with_the_task_name_and_keeps_its_run_status() {
  ...
  printf '%s' "$out" | jq -e '
    (.sections.underway.rows | length) == 1
      and (.sections.underway.rows[0]
...
multi_repo_payload() {
  jq -n '
      ...
      + (if $type == "merge" then {risk:"low"} else {} end);
    {
      ...
      prs_live:false,
      # An object-construction value in jq is a RESTRICTED expression, so the
      # concatenation below has to be parenthesised as a whole. jq 1.8 tolerates
      # it bare; the jq CI runs on does not, and rejected the whole program.
      captains_call:
