...
#                this safe for the crew shape, where the nested worktree
#                shell fm-spawn.sh starts with
#                `(cd -- '<wt>' && exec $SHELL)` sits under the pane's top
#                shell.
...
# discovery after the pane enters its worktree.
#
...
# shell `cd`s or enters a subshell (as fm-spawn.sh's
# `(cd -- '<wt>' && exec $SHELL)` does). Reading it here
...
# process's cwd instead, which is what changes when that command enters its
...
# spawn-time commands (the worktree-entry subshell, the GOTMPDIR export).
# `pane run` types
