...
# (bin/fm-home-seed.sh) and for crewmate spawns (bin/fm-spawn.sh), which take
# the lease themselves rather than letting the pane's subshell hold the slot.
# The lease stops the pool from handing a task's copy to a later spawn, which
# is the question the pool itself answers; it does not answer the other one,
# at teardown: whether the slot beside a task's record is still that task's
# own. A slot Treehouse reports as free - the process lease it also keeps
# (owner_pid plus owner_started_at) lapses the moment the worker exits - reads
# identical whether it is still this task's or has since been handed to
# another one. Firstmate therefore keeps its own claim on top: one file naming
# the task that took the slot, written by bin/fm-spawn.sh under the same
