...
freshen_spawn_worktree_base() {  # <worktree>
  ...
  # an effort accumulate on one integration branch without editing the shared
  # registry and having to remember to put it back. Absent that, the project's
  # base= record in data/projects.md names the right branch, and absent that too
  # the repository default stands.
  if [ -n "$BASE_ARG" ]; then
    default=$BASE_ARG
  ...
  # Recorded below as base= so recovery, relaunch, and teardown read the branch
  # this task was actually created against rather than re-resolving a registry
