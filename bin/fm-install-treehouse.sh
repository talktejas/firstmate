...
FM_TREEHOUSE_CI_DOWNLOAD_BACKOFF_SECONDS=2
...
attempt=1
while :; do
  if curl -fsSL --max-filesize "$FM_TREEHOUSE_CI_MAX_BYTES" "$URL" -o "$TMP/$ARCHIVE" \
    2>"$TMP/curl-error"; then
    break
  fi
  if [ "$attempt" -ge "$FM_TREEHOUSE_CI_DOWNLOAD_ATTEMPTS" ]; then
    last_curl_error=$(tail -n 1 "$TMP/curl-error")
    [ -n "$last_curl_error" ] || last_curl_error='curl exited without a diagnostic'
    die "download failed for $URL after $attempt attempts (bounded at $FM_TREEHOUSE_CI_MAX_BYTES bytes); last curl error: $last_curl_error"
  fi
  printf 'fm-install-treehouse.sh: download attempt %s/%s failed; retrying in %ss\n' \
    "$attempt" "$FM_TREEHOUSE_CI_DOWNLOAD_ATTEMPTS" "$FM_TREEHOUSE_CI_DOWNLOAD_BACKOFF_SECONDS" >&2
  rm -f "$TMP/$ARCHIVE"
  sleep "$FM_TREEHOUSE_CI_DOWNLOAD_BACKOFF_SECONDS"
  attempt=$((attempt + 1))
done
