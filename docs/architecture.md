...
The spawn records the base it resolved in the task's metadata and a relaunch reuses that record, so a task keeps the branch it was created against even after the project's own record moves on.
That record also governs the landing and the cleanup: `bin/fm-merge-local.sh` fast-forwards the task's own recorded base and `bin/fm-teardown.sh` measures its merged work against it, falling back to the project's standing base only for a task that recorded none - otherwise a single task would land the whole integration branch into the standing branch, and cleanup would call already-merged work unmerged.
