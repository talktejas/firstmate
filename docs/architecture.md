...
Either base resolves to `origin/<branch>` for a shared clone and to the local branch for a `local-only` project, which lands through `bin/fm-merge-local.sh` and never pushes; a spawn that carries no mode of its own - every scout, since `--mode` is refused there - reads the project's registered posture for that decision rather than treating the missing mode as a shared clone.
