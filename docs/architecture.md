...
The registry also records an optional base branch: it is what a task worktree must start from, because a freshly allocated pool worktree otherwise lands on the repo's default branch, which is the wrong base for a project developed elsewhere; `bin/fm-spawn.sh` checks that branch out and refuses to launch a worker onto a base it cannot confirm.
That registry record is the project's standing base, not one task's: `bin/fm-spawn.sh --base <branch>` overrides it for a single ship or scout spawn without writing anything back, so an effort that must accumulate on one integration branch never edits shared state it then has to remember to restore.
The spawn records the base it resolved in the task's metadata and a relaunch reuses that record, so a task keeps the branch it was created against even after the registry moves on.
