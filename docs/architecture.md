...
A fourth, CAPTAIN INBOX NOTES, lists every captain note still unread under `state/inbox/` on the same drains regardless of whether its one wake was acknowledged; [`command-center.md`](command-center.md) owns that contract.
...
A task's pool worktree is claimed with a durable treehouse lease labelled by the task record that owns it, so a copy whose agent is not running is never handed to the next spawn and reset under it, and teardown's return releases the claim.
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns ship/scout worktree isolation, the lease claim, and fresh-base refusal rules, including spawns from linked homes.
...
A project's development branch is declared in the project repository itself, in a `.firstmate-base` file holding the branch name, so every home that clones the project reads the same answer without a registry entry of its own; `bin/fm-project-base.sh` is the one resolver, reading that file from whichever branch carries it and falling back to the registry's legacy `base=` record and then to the repository default.
That branch is what a task worktree must start from, because a freshly allocated pool worktree otherwise lands on the repo's default branch; `bin/fm-spawn.sh` checks the resolved branch out and refuses to launch a worker onto a base it cannot confirm.
It is also what `bin/fm-fleet-sync.sh` keeps each clone current against, so a clone of a project developed off its repository default cannot report itself current while falling months behind.
For a `local-only` project it is equally where work lands: `bin/fm-merge-local.sh` fast-forwards the resolved branch and `bin/fm-teardown.sh` measures merged work against that same branch, so the landing and the cleanup check can never disagree about which branch holds the work.
That resolved branch is the project's standing base, not one task's: `bin/fm-spawn.sh --base <branch>` overrides it for a single ship or scout spawn without writing anything back, so an effort that must accumulate on one integration branch never edits shared state it then has to remember to restore.
The spawn records an explicitly dispatched base in the task's metadata and a relaunch reuses that record, so such a task keeps the branch it was created against even after the project's own record moves on; a spawn that named no base records none and keeps resolving the project's standing declaration, which stays free to change under it.
That record also governs the landing and the cleanup: `bin/fm-merge-local.sh` fast-forwards the task's own recorded base and `bin/fm-teardown.sh` measures its merged work against it, falling back to the project's standing base when the task recorded none or when the branch it recorded has since disappeared - otherwise a single task would land the whole integration branch into the standing branch, and cleanup would call already-merged work unmerged.
Either base resolves to `origin/<branch>` for a shared clone and to the local branch for a `local-only` project, which lands through `bin/fm-merge-local.sh` and never pushes; an explicit `--base` on a spawn that carries no mode of its own - a scout, since `--mode` is refused there - reads the project's registered posture for that decision rather than treating the missing mode as a shared clone.
...
A pool worktree is only returned after teardown passes the slot-ownership proof: a supported live endpoint, or a contradictory task record the slot's own claim cannot rule this record out against, refuses without touching either task, and no discard authority relaxes that.
...
That claim is also what breaks a two-record collision, which would otherwise refuse in both directions and strand the pair: the record the claim does not name is provably not the owner, so it retires through that same reassigned path and leaves the slot with one record.
...
Every clone is measured against its declared development branch rather than the repository default, resolved through `bin/fm-project-base.sh`; a declared branch that `origin` does not carry falls back to the repository default and says so on stdout, and every verdict for that clone is qualified so it cannot read as plainly current.
A clone already on that branch fast-forwards to `origin/<branch>`, and two drifts auto-recover because neither can strand work: a clean detached HEAD holding no unique commits is re-attached, and a clean clone sitting on the repository's own default branch with no unpublished commit is moved onto the declared branch, without which every clone of such a project would stay stuck forever rather than self-heal.
Dirty clones, any other named branch, detached HEADs with unique commits, diverged branches, and target branches checked out in another worktree are reported as `STUCK:` with their behind count and left untouched, because a named branch may be where the captain deliberately parked the clone.
