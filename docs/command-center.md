...
**Messages** is the default and is what firstmate said to you: one row per message, newest first, each with its title and its time. A captured message records no project, worktree or branch - the conversation record does not say which project a sentence is about - so those read "Not recorded"; a message written by hand with `bin/fm-captain-message.sh --task` carries all three. `bin/fm-captain-message-backfill.py` can fill a legacy row only when that row already carries a task id whose task record still names the project and worktree; it never guesses from message text, and never derives a past branch from a worktree's current one.
