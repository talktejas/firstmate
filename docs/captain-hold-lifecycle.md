...
`verify` requires the recorded attestation, requires every recorded inventory entry to still be durable (actively captain-held, carrying a recorded answer, or pruned with a matching answer receipt), and fails on any keyed status decision that opened after the last `complete`, which makes re-running `complete` the repair.
...
Its completion-replacement cases prove that an answered inventory still verifies, a corrective `complete` that drops only answered ids replaces the stored list so teardown proceeds, a dropped id still held and unanswered is refused by name, a dropped id with no row is cleared only with `--repair-reason` as reported drift, a pruned answered call verifies through its receipt while a never-existent id still refuses, a re-held call's second closing answer replaces its receipt, and a drop is never spent as drift while the backlog is unreadable or its bounded listing read times out, while a completion that drops nothing never reads the listing.
...
The markdown-to-beads migration family runs the same suite's beads fixture (bd-driven scratch graph, self-skipping on markdown-only tasks-axi installs) and proves: `verify` and `complete` resolve an attested legacy id through a migrated row's marker note, through the configured prefix when no row carries a note - naming the resolved row in the completion line - and through the marker note of a pre-collapse derived identity; a marker-noted row wins over an unrelated captain-held row occupying the bare prefix namesake; an unresolvable id is refused once naming the id (never an empty name); and the attested id stays in `decision_keys=` for idempotent re-verification, while a later pass naming the same row by its migrated id replaces that spelling rather than reading as a dropped call.
