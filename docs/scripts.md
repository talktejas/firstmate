...
| `command-center.py`      | Serve the captain's permanent command center page and answer endpoints on loopback |
...
| `fm-captain-message.sh`  | Record one message firstmate sent the captain in the durable log the command center reads |
...
| `fm-delegate-pretool-check.sh` | Primary-home project-work PreToolUse guard: investigate and build through workers, not in place (docs/delegate-guard.md) |
...
| `fm-merge-local.sh`      | Fast-forward a `local-only` task's own local base branch after approval              |
