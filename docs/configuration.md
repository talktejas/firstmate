...
FM_CHECK_TIMEOUT=30     # seconds allowed per slow check script, except the contributions check
FM_CONTRIBUTIONS_CHECK_TIMEOUT=138   # seconds allowed for the contributions check alone; it reads a forge eight times for one pull request
FM_CONTRIBUTIONS_BUDGET=   # seconds one contributions poll may spend on forge reads; unset takes FM_CONTRIBUTIONS_CHECK_TIMEOUT minus three, a larger value is cut to it
FM_CONTRIBUTIONS_CALL_TIMEOUT=15   # seconds allowed for one gh call inside a contributions poll
...
FM_TREEHOUSE_GET_TIMEOUT=60               # seconds bounding the leased pool-worktree allocation bin/fm-spawn.sh runs under the shared treehouse project lock; hitting the bound refuses the spawn, releases that lock, and names the holder label a half-created slot would carry so it can be inspected before release
