...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  The same marker guard carries every tracked `.claude/settings.json` entry whose event Grok already covers through its own `.grok/hooks/` registration, which is both `Stop` entries, the `SessionStart` entry, and the two `PreToolUse` Bash entries; `bin/fm-subagent-pretool-check.sh` and `bin/fm-delegate-pretool-check.sh` are the two deliberate unguarded exceptions because no Grok registration covers either event, recorded in the "Known residual gap" sections of [`subagent-guard.md`](subagent-guard.md) and [`delegate-guard.md`](delegate-guard.md).
