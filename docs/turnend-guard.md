...
Their separate owners are [`arm-pretool-check.md`](arm-pretool-check.md), [`cd-guard.md`](cd-guard.md), [`subagent-guard.md`](subagent-guard.md), and [`delegate-guard.md`](delegate-guard.md).
...
`bin/fm-watch.sh` touches `state/.last-watcher-beat` once per cycle, immediately before its terminal wait (`event_wait_or_sleep`) as well as at the top of the next cycle, and again before each check in a slow-check sweep so the grace covers the longest single check rather than the whole sweep, so a healthy watcher's beacon can legitimately age up to `FM_POLL` seconds between touches.
...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  The same marker guard carries every tracked `.claude/settings.json` entry whose event Grok already covers through its own `.grok/hooks/` registration, which is both `Stop` entries, the `SessionStart` entry, the two `PreToolUse` Bash entries, and the delegate guard's `PreToolUse` entry; `bin/fm-subagent-pretool-check.sh` is the one deliberate unguarded exception because no Grok registration covers that event, recorded in the "Known residual gap" section of [`subagent-guard.md`](subagent-guard.md); `bin/fm-delegate-pretool-check.sh` carries its own `.grok/hooks/` registration for the shell surface and so is marker-guarded like the rest, with Grok's read and write tools left unwired as [`delegate-guard.md`](delegate-guard.md) records.
